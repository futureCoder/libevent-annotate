# 环境安装
1. 从github下载源码[libevent-release-2.1.12-stable.tar.gz](https://github.com/libevent/libevent/releases/tag/release-2.1.12-stable)，并解压
2. `/home/opt/cmake-3.17.2/bin/cmake -Bbuild -DCMAKE_INSTALL_PREFIX=output -DCMAKE_C_COMPILER=/opt/compiler/gcc-10/bin/gcc -DEVENT__DISABLE_SAMPLES=ON -DEVENT__DISABLE_TESTS=ON -DEVENT__DISABLE_REGRESS=ON -DEVENT__LIBRARY_TYPE=STATIC -DEVENT__DISABLE_BENCHMARK=ON -DEVENT__DISABLE_MM_REPLACEMENT=ON -DEVENT__DISABLE_DEBUG_MODE=ON`
3. `/home/opt/cmake-3.17.2/bin/cmake --build build -v &>compile.log`
4. `/home/opt/cmake-3.17.2/bin/cmake --install build &>install.log`
5. 保存config文件`cp build/include/evconfig-private.h . && cp build/include/event2/event-config.h include/event2`

# patch
1. 去掉`include/event2/event-config.h`中的`EVENT__HAVE_SYS_RANDOM_H`和`EVENT__HAVE_GETRANDOM`，改为gcc10以上启用，适配厂内环境
